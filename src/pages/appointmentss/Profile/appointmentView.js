...
const UserViewLeft = () => {
  ...
  const { data, loading, execute } = useAsync((appiontd) => axios.get(`${baseUrl}/api/appointments/${appiontd}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }), { immediate: false })
    ...
    return (
      <Grid container spacing={12}>
        <Grid item xs={12} lg={8}>
          <Card>
            <Typography sx={{ m: 8, fontWeight: 'bold', color: 'text.secondary', textAlign: 'center' }}>Appointment  Details </Typography>
            <CardContent sx={{ pb: 4 }}>
              <Box sx={{ pt: 4 }}>
