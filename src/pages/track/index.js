...
const TrackLawyer = ({ apiData }) => {
  ...
  const { data, execute: fetchLawyerData, status, loading } = useAsync(() => axios.get(`${baseUrl}/api/admin/lawyerTrack?page=${page + 1}&limit=${pageSize}&search=${value}&is_active=${statusValue}`, {
    headers: {
  ...
  useEffect(() => {
    if (data?.lawyers?.totalDocs) {
      setTotalCounts(data?.lawyers?.totalDocs)
    }
  }, [data])
  ...
  const changeRowDataKeys = (data) => {
    const newArr = data?.map((obj) => {
      ...
      Object.keys(obj).forEach((key) => {
        ...
        } else if (key === "numberOfConsultations") {
          newObj.numberOfConsultations = obj.numberOfConsultations;
        } else if (key === "numberOfCompleted") {
          newObj.numberOfCompleted = obj.numberOfCompleted;
        } else if (key === "numberOfCanceled") {
          newObj.numberOfCanceled = obj.numberOfCanceled;
      ...
      const order = ["_id", "full_name", "numberOfConsultations", "numberOfCompleted", "numberOfCanceled", "avgRating"];
  ...
  const onBtnExport = () => convertJsonToExcel(changeRowDataKeys(data?.lawyers?.docs), "Lawyer");
  ...
  const columns = [
    ...
    },
    {
      flex: 0.10,
      minWidth: 100,
      headerName: 'Num of sessions',
      field: 'numberOfConsultations',
      headerClassName: 'super-app-theme--header',
      renderHeader: () => <Typography sx={{
        fontSize: '18px', color: '#161616', fontWeight: 700, textWrap: 'wrap'
      }}>{'Num of sessions'}</Typography>,
      renderCell: ({ row }) => {
        return (
          <Typography noWrap sx={{ fontWeight: 500, color: '#000', textTransform: 'capitalize' }}>
            {row?.numberOfConsultations}
          </Typography>
        )
      }
    },
    {
      ...
      headerName: 'Num Of Sessions',
      headerClassName: 'super-app-theme--header',
      field: 'numberOfCompleted',
      renderHeader: () => <Typography sx={{
        fontSize: '18px', color: '#161616', fontWeight: 700,
        textWrap: 'wrap'
      }}>{'Num of Sessions '} (<span style={{ color: '#229C5B' }}>Success</span>)
      </Typography>,
      renderCell: ({ row }) => {
        return (
          <Typography noWrap sx={{ fontWeight: 500, color: '#000', textTransform: 'capitalize' }}>
            {row?.numberOfCompleted}
          </Typography>
        )
      }
    },
    {
      ...
      headerName: 'Num of Sessions',
      headerClassName: 'super-app-theme--header',
      field: 'numberOfCanceled',
      renderHeader: () => <Typography sx={{
        fontSize: '18px', color: '#161616', fontWeight: 700,
        textWrap: 'wrap'
      }}>{'Num of Sessions '} (<span style={{ color: '#BA1F1F' }}>Failed</span>)</Typography>,
      renderCell: ({ row }) => {
        return (
          <Typography noWrap sx={{ fontWeight: 500, color: '#000', textTransform: 'capitalize' }}>
            {row?.numberOfCanceled}
          </Typography>
        )
      }
    },
    {
      ...
      field: 'avgRating',
      renderHeader: () => <Typography sx={{
        fontSize: '17px', color: '#161616', fontWeight: 700,
      }}>{'Rate (AVG)'}</Typography>,
      renderCell: ({ row }) => {
        return (
          <Typography noWrap sx={{ fontWeight: 500, color: '#000', textTransform: 'capitalize' }}>
      ...
      renderCell: ({ row }) => {
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Tooltip title='Lawyer Track Details'>
              <IconButton size='small' sx={{ color: 'text.labelBlack' }} onClick={() => handleShowTrack(row?.id)}>
                <Icon icon='ph:eye' />
              </IconButton>
            </Tooltip>
          </Box>
  ...
  return (
    <Grid container spacing={4}>
        ...
        <Card>
          ...
          >
            <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
              <Typography variant='h1' sx={{ fontSize: '2rem', fontWeight: 400, color: '#000' }}>Lawyer Track</Typography>
            ...
            <Box sx={{ rowGap: 2, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 4 }}>
              ...
              <Button sx={{
                ...
                px: '16px',
                color: '#64656980'
              }} variant='outlined' startIcon={<Icon icon='tabler:upload' />} onClick={() => onBtnExport()} disabled={!data?.lawyers?.docs}>
                Export
              </Button>
              {/* <Select
                IconComponent={"span"}
      ...
      <TrackDetails open={openDetails} onClose={handleClose} trackId={trackId} fetchData={fetchLawyerData} />
      <SidebarLawyer open={addUserOpen} toggle={toggleSidebarLawyer} fetchLawyerData={fetchLawyerData} />
    </Grid>
  )
}

export default TrackLawyer
